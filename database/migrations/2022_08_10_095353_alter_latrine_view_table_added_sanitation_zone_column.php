...
return new class extends Migration
{
    ...
    private function createView(): string
    {
        ...
        SELECT
            `lt`.*, `cmp`.`name` AS `camp_name`, `blk`.`short_name` AS `block_name`, `sblk`.`short_name` AS `sub_block_name`, `ft`.`name` AS `facility_type_name`, `fu`.`name` AS `facility_used_name`, `cns`.`name` AS `construction_status_name`, `lst`.`name` AS `latrine_structure_name`, `pt`.`name` AS `pit_type_name`, `dnr`.`name` AS `donor_name`, `sz`.`name` AS `sanitation_zone_name`,
            ...
            datediff(DATE_ADD(MAX(DATE(des.collection_date)), INTERVAL FLOOR(TIMESTAMPDIFF(DAY, min(des.collection_date), max(des.collection_date) ) / (count(*) - 1)) DAY), now()) AS days_left,
            (SELECT total_volume_desludged FROM desludges WHERE desludges.latrine_id = lt.id AND desludges.id = (SELECT MAX(id) FROM desludges where desludges.latrine_id = lt.id)) AS last_volume_desludged
        FROM `latrines` as `lt`
        ...
        LEFT JOIN `sanitation_zones` as `sz` on `lt`.`sanitation_zone_id` = `sz`.`id`
